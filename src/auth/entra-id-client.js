...
const getEntraIdAuthConfig = async (oidcConfigurationUrl) => {
...
const getEntraIdRefreshToken = async (refreshUrl, params) => {
  ...
  if (res.statusCode === httpConstants.HTTP_STATUS_OK) {
    try {
      ...
      logger.error(e, 'Response from Entra ID refresh call contains invalid JSON payload.')
    }
  }
...
export {
  getEntraIdAuthConfig,
  getEntraIdRefreshToken
}
