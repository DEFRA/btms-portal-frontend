...
const getChecks = (preNotification, complementParameterSet) => {
  ...
  const iuuDecision =
    (needsCatchCertificate &&
      iuuDecisionDisplay[
        preNotification.partTwo?.controlAuthority?.iuuOption
      ]) ||
    DECISION_NOT_GIVEN
...
const chedIdentifier = (term) => {
  return term.substring(term.lastIndexOf('.')+1)
}

const searchTermMatcher = (searchTerm, preNotification) =>
  chedIdentifier(preNotification.referenceNumber) === chedIdentifier(searchTerm)
