...
test.each([
  ...
  'Should show the admin view page',
  async ({ resourceType, searchTerm, type }) => {
    ...
    const { payload } = await server.inject({
      method: 'get',
      url: `${paths.ADMIN_VIEW}?${queryStringParams.SEARCH_TERM}=${searchTerm}&${queryStringParams.TYPE}=${type}`,
      auth: {
        strategy: 'session',
        credentials
      }
...
test('Should deserialise nested JSON', async () => {
  ...
  const { payload } = await server.inject({
    method: 'get',
    url: `${paths.ADMIN_VIEW}?${queryStringParams.SEARCH_TERM}=CHEDPP.GB.2025.1010001&${queryStringParams.TYPE}=information`,
    auth: {
      strategy: 'session',
      credentials
    }
...
test('Should show an error with an invalid search term', async () => {
  ...
  const { payload } = await server.inject({
    method: 'get',
    url: `${paths.ADMIN_VIEW}?${queryStringParams.SEARCH_TERM}=abcd&${queryStringParams.TYPE}=information`,
    auth: {
      strategy: 'session',
      credentials
    }
...
test('Should show a not found error with a search term that could not be found', async () => {
  ...
  const { payload } = await server.inject({
    method: 'get',
    url: `${paths.ADMIN_VIEW}?${queryStringParams.SEARCH_TERM}=24GBBGBKCDMS895999&${queryStringParams.TYPE}=information`,
    auth: {
      strategy: 'session',
      credentials
    }
